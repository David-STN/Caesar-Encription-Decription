...
int decryption(string alphabet, string clear_text, string cif_text){
    ...
    for(int i=0; i<cif_text.size(); i++){
        for (int k=0;k<alphabet.size(); k++){
            ...
            if(first == second){
                clear_text.at(i) = alphabet.at(k+1);
                cout << "The decrypted letter is: " << clear_text.at(i) << endl;
                break;
                }
            }
        }
        ...
        clear_text.at(j)= toupper(letter);
...
int main()
{
    ...
    string clear_text;
    decryption(Ana,clear_text,cif_text);
    return 0;
}
